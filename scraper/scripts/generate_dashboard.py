...
def render_html(series: Dict[str, Any], highlights: Dict[str, Any] | None = None, weekly_link: str | None = None) -> str:
    ...
    weekly_html = f"<a id=\"weekly_link\" href=\"{weekly_link}\" target=\"_blank\">View weekly summary</a>" if weekly_link else ""
    html = f"""
<!DOCTYPE html>
<html lang=\"en\">
<head>
...
<body>
  ...
  <div class=\"row\">
    <div class=\"card\">
      ...
      <div style=\"margin-top:8px\">{weekly_html}</div>
    </div>
  </div>
  <div class=\"row\">
    <div class=\"card\">
  ...
  <script>
    ...
    function fmtDelta(d) {
      if (d == null) return '';
      const s = (d >= 0 ? '+' : '') + d.toFixed ? d.toFixed(2) : d;
      return ` (${s})`;
    }
